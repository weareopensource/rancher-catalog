...
retry_until() {
    ...
    until [[ $(mongo admin --host "${host}" "${creds[@]}" "${tls_args[@]}" --quiet --eval "${command}" | tail -n1) == "${expected}" ]]; do
        sleep 1
...
shutdown_mongo() {
    ...
    if (! mongo admin --host "${host}" "${admin_creds[@]}" "${tls_args[@]}" --eval "db.shutdownServer({$args})"); then
...
init_mongod_standalone() {
    ...
    mongo admin --port "${port}" "${admin_creds[@]}" "${tls_args[@]}" /init/initMongodStandalone.js
...
sed -i '/\|-/d' /data/configdb/mongod.conf # rancher security toyaml twite probably
...
for peer in "${peers[@]}"; do
    ...
    if [[ $(mongo admin --host "${peer}" "${admin_creds[@]}" "${tls_args[@]}" --quiet --eval "rs.status().myState" | tail -n1) == "1" ]]; then
...
if [[ "${primary}" = "${service_name}" ]]; then
    ...
    if [[ $(mongo admin --host "${primary}" "${admin_creds[@]}" "${tls_args[@]}" --quiet --eval "rs.conf().members.findIndex(m => m.host == '${service_name}:${port}')" | tail -n1) == "-1" ]]; then
      log "Adding myself (${service_name}) to replica set..."
      if (mongo admin --host "${primary}" "${admin_creds[@]}" "${tls_args[@]}" --eval "rs.add('${service_name}')" | grep 'Quorum check failed'); then
...
elif (mongo "${tls_args[@]}" --eval "rs.status()" | grep "no replset config has been received"); then
    log "Initiating a new replica set with myself ($service_name)..."
    mongo "${tls_args[@]}" --eval "rs.initiate({'_id': '$replica_set', 'members': [{'_id': 0, 'host': '$service_name'}]})"
    ...
    if [[ "${AUTH}" == "true" ]]; then
        ...
        mongo admin "${tls_args[@]}" --eval "db.createUser({user: '${admin_user}', pwd: '${admin_password}', roles: [{role: 'root', db: 'admin'}]})"
    fi
fi

# User creation
if [[ -n "${primary}" && "$AUTH" == "true" && "$METRICS" == "true" ]]; then
    metric_user_count=$(mongo admin --host "${primary}" "${admin_creds[@]}" "${tls_args[@]}" --eval "db.system.users.find({user: '${metrics_user}'}).count()" --quiet | tail -n1)
    if [[ "${metric_user_count}" == "0" ]]; then
        log "Creating clusterMonitor user..."
        mongo admin --host "${primary}" "${admin_creds[@]}" "${tls_args[@]}" --eval "db.createUser({user: '${metrics_user}', pwd: '${metrics_password}', roles: [{role: 'clusterMonitor', db: 'admin'}, {role: 'read', db: 'local'}]})"
    fi
fi
